...
public class MainActivity extends Activity {
    ...
    public void showDashboard(){base("Dashboard");JSONArray records=getRecords();LinearLayout hero=card("This Year");hero.addView(tv(records.length()+" expense record"+(records.length()==1?"":"s"),24,WHITE,Typeface.BOLD));hero.addView(tv("Local records are saved on this phone. Google Sheets sync is connected. Yearly sheets are formatted with title, frozen/filter headers, and alternating row colors.",14,MUTED,Typeface.NORMAL));
    ...
    void fillFromReceipt(String raw){String[] lines=raw.split("\n");String vendor="";for(String l:lines){String x=l.trim();String low=x.toLowerCase(Locale.US);if(x.length()>2&&!x.matches(".*\\d{3,}.*")&&!low.contains("total")&&!low.contains("amount")&&!low.contains("receipt")&&!low.contains("date")&&!low.contains("visa")&&!low.contains("mastercard")){vendor=x;break;}}if(!vendor.isEmpty())addVendor.setText(vendor);String amount=findAmount(raw);if(!amount.isEmpty())addAmount.setText(amount);String date=findDate(raw);if(!date.isEmpty())addDate.setText(date);else Toast.makeText(this,"Receipt date was not clear. Please confirm expense date.",Toast.LENGTH_LONG).show();String guessed=guessCategory(raw+" "+vendor);String[] allCats=getCategories();for(int i=0;i<allCats.length;i++){if(allCats[i].equals(guessed)){addCat.setSelection(i);break;}}if(addTitle.getText().toString().trim().isEmpty())addTitle.setText(vendor.isEmpty()?guessed+" receipt":vendor+" - "+guessed);if(addDesc.getText().toString().trim().isEmpty())addDesc.setText("Business expense recorded from receipt for Virtual Beehive Inc.");}
    String findAmount(String raw){Matcher m=Pattern.compile("(?i)(grand total|total due|total|amount|balance)[^0-9$]{0,30}\\$?([0-9]{1,5}(?:,[0-9]{3})*\\.[0-9]{2})").matcher(raw);String last="";while(m.find())last=m.group(2).replace(",","");if(!last.isEmpty())return last;m=Pattern.compile("\\$\\s*([0-9]{1,5}(?:,[0-9]{3})*\\.[0-9]{2})").matcher(raw);while(m.find())last=m.group(1).replace(",","");if(!last.isEmpty())return last;m=Pattern.compile("([0-9]{1,5}\\.[0-9]{2})").matcher(raw);while(m.find())last=m.group(1);return last;}
    String findDate(String raw){Matcher m=Pattern.compile("(20\\d{2})[/-](\\d{1,2})[/-](\\d{1,2})").matcher(raw);if(m.find())return String.format(Locale.US,"%04d-%02d-%02d",Integer.parseInt(m.group(1)),Integer.parseInt(m.group(2)),Integer.parseInt(m.group(3)));m=Pattern.compile("(\\d{1,2})[/-](\\d{1,2})[/-](\\d{2,4})").matcher(raw);if(m.find()){int y=Integer.parseInt(m.group(3));if(y<100)y+=2000;return String.format(Locale.US,"%04d-%02d-%02d",y,Integer.parseInt(m.group(1)),Integer.parseInt(m.group(2)));}m=Pattern.compile("(?i)(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\\s+(\\d{1,2}),?\\s+(20\\d{2})").matcher(raw);if(m.find()){String[] ms={"jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"};int mo=1;for(int i=0;i<ms.length;i++)if(m.group(1).toLowerCase(Locale.US).startsWith(ms[i]))mo=i+1;return String.format(Locale.US,"%04d-%02d-%02d",Integer.parseInt(m.group(3)),mo,Integer.parseInt(m.group(2)));}return "";}
    ...
    String getOrCreateSheet(String token,String year)throws Exception{String key="sheet_"+year;String id=sp.getString(key,"");boolean created=false;if(id.isEmpty()){JSONObject body=new JSONObject();body.put("name","VB expenses "+year);body.put("mimeType","application/vnd.google-apps.spreadsheet");body.put("parents",new JSONArray().put(DRIVE_FOLDER_ID));JSONObject res=new JSONObject(http("POST","https://www.googleapis.com/drive/v3/files",token,body.toString()));id=res.getString("id");sp.edit().putString(key,id).apply();created=true;}ensureSheetFormatted(token,id,year,created);return id;}
    JSONArray headerRow(){return new JSONArray(Arrays.asList("Reference Number","Date Entered","Entered By","Position","Expense Date","Year","Expense Category","Frequency","Title","Description / Business Purpose","Vendor / Store","Amount","Receipt URI","Company","Status"));}
    void ensureSheetFormatted(String token,String sheetId,String year,boolean created)throws Exception{String a1="";try{String res=http("GET","https://sheets.googleapis.com/v4/spreadsheets/"+sheetId+"/values/Sheet1!B1:B1",token,null);JSONObject o=new JSONObject(res);JSONArray vals=o.optJSONArray("values");if(vals!=null&&vals.length()>0)a1=vals.getJSONArray(0).optString(0);}catch(Exception ignored){}boolean already=a1.contains("Virtual Beehive Inc.");if(already)return;if(!created){JSONObject insert=new JSONObject().put("requests",new JSONArray().put(new JSONObject().put("insertDimension",new JSONObject().put("range",new JSONObject().put("sheetId",0).put("dimension","ROWS").put("startIndex",0).put("endIndex",2)).put("inheritFromBefore",false))));http("POST","https://sheets.googleapis.com/v4/spreadsheets/"+sheetId+":batchUpdate",token,insert.toString());}
        JSONArray values=new JSONArray();values.put(new JSONArray(Arrays.asList("=IMAGE(\"https://drive.google.com/uc?export=view&id="+getOrUploadLogo(token)+"\",1)","Virtual Beehive Inc. Expense Bookkeeping - "+year,"","","","","","","","","","","","","")));values.put(new JSONArray(Arrays.asList("","Private company expense log. Records are append-only; corrections should be added as new notes/rows.","","","","","","","","","","","","","")));values.put(headerRow());JSONObject body=new JSONObject().put("values",values);http("PUT","https://sheets.googleapis.com/v4/spreadsheets/"+sheetId+"/values/Sheet1!A1:O3?valueInputOption=USER_ENTERED",token,body.toString());formatSheet(token,sheetId);}
    String getOrUploadLogo(String token)throws Exception{String saved=sp.getString("driveLogoId","");if(!saved.isEmpty())return saved;ByteArrayOutputStream baos=new ByteArrayOutputStream();BitmapFactory.decodeResource(getResources(),getResources().getIdentifier("vb_logo","drawable",getPackageName())).compress(Bitmap.CompressFormat.PNG,100,baos);String boundary="vbBoundary"+System.currentTimeMillis();JSONObject meta=new JSONObject();meta.put("name","Virtual Beehive Inc logo.png");meta.put("parents",new JSONArray().put(DRIVE_FOLDER_ID));String start="--"+boundary+"\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n"+meta.toString()+"\r\n--"+boundary+"\r\nContent-Type: image/png\r\n\r\n";String end="\r\n--"+boundary+"--";HttpURLConnection c=(HttpURLConnection)new URL("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart").openConnection();c.setRequestMethod("POST");c.setRequestProperty("Authorization","Bearer "+token);c.setRequestProperty("Content-Type","multipart/related; boundary="+boundary);c.setDoOutput(true);OutputStream os=c.getOutputStream();os.write(start.getBytes("UTF-8"));os.write(baos.toByteArray());os.write(end.getBytes("UTF-8"));os.close();int code=c.getResponseCode();String res=readAll(code>=200&&code<300?c.getInputStream():c.getErrorStream());if(code<200||code>=300)throw new Exception("Logo upload HTTP "+code);String id=new JSONObject(res).getString("id");try{JSONObject perm=new JSONObject().put("type","anyone").put("role","reader");http("POST","https://www.googleapis.com/drive/v3/files/"+id+"/permissions",token,perm.toString());}catch(Exception ignored){}sp.edit().putString("driveLogoId",id).apply();return id;}
